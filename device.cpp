...
//////////////////////////////
vector<unsigned char> hmac_sha256_trunc(const vector<unsigned char> &key,
                                        const vector<unsigned char> &msg,
                                        size_t out_bytes = HMAC_TRUNC_BYTES) {
  unsigned int len = EVP_MAX_MD_SIZE;
  vector<unsigned char> mac(EVP_MAX_MD_SIZE);
  HMAC(EVP_sha256(), key.data(), (int)key.size(), msg.data(), msg.size(),
       mac.data(), &len);
  if (out_bytes > len)
    out_bytes = len;
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--mode" && i + 1 < argc)
      mode = argv[++i];
    else if (a == "--device-id" && i + 1 < argc)
      device_id = argv[++i];
    else if (a == "--port" && i + 1 < argc)
      port = stoi(argv[++i]);
    else if (a == "--flip-prob" && i + 1 < argc)
      flip_prob = stod(argv[++i]);
    else if (a == "--cache-max" && i + 1 < argc)
      cache_max = stoul(argv[++i]);
  ...
  while (true) {
    ...
    for (int i = 3; i >= 0; --i)
      msg.push_back((unsigned char)((ts >> (i * 8)) & 0xFF));
